...
WITH cte AS (
	SELECT 	customer_id, 
			order_date,
			LAG(order_date) OVER (PARTITION BY customer_id ORDER BY order_date) AS prev_odate
	FROM @Orders)
SELECT customer_id, order_date, prev_odate  
FROM cte
WHERE  DATEDIFF(DAY, prev_odate, order_date) = 1

/*
	customer_id		order_date 	prev_odate
	---------------------------------------
	AB_12345		2025-06-24	2025-06-23
	AB_12345		2025-06-25	2025-06-24
	ZD_9235			2025-06-26	2025-06-25
	ZD_9235			2025-06-27	2025-06-26
